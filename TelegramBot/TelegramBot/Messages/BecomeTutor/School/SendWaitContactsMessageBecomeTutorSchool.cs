...
namespace TelegramBot.Messages.BecomeTutor.School
{
    ...
    {
        public static async Task SendWaitContactsMessageBecomeTutorSchool(ITelegramBotClient botClient, Update? update, Update updateUserMessage, CancellationToken cancellationToken, string description, string fullname, string subjId, string categoryId)
        {
            var chatId = new long();
            ...
            if (update == null)
            {
                ...
                InlineKeyboardMarkup inlineKeyboard = new(new[]
                {
                    new []
                    {
                        InlineKeyboardButton.WithCallbackData(text: "Назад", callbackData: $"become_tutor_wait_description_school&{fullname}&{subjId}&{categoryId}")
                    }
                });
                ...
                Users.previousMessage[chatId].CallbackQuery.Data = $"become_tutor_wait_contacts_school&{description}&{fullname}&{subjId}&{categoryId}";
