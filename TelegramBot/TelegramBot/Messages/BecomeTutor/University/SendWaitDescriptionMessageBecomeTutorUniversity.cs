...
namespace TelegramBot.Messages.BecomeTutor.University
{
    ...
    {
        public static async Task SendWaitDescriptionMessageBecomeTutorUniversity(ITelegramBotClient botClient, Update? update, Update updateUserMessage, CancellationToken cancellationToken, string fullname, string subjId, string categoryId)
        {
            var chatId = new long();
            ...
            if (update == null)
            {
                ...
                InlineKeyboardMarkup inlineKeyboard = new(new[]
                {
                    new []
                    {
                        InlineKeyboardButton.WithCallbackData(text: "Назад", callbackData: $"become_tutor_wait_fio_university&{subjId}&{categoryId}"),
                    }
                });
                ...
                Users.previousMessage[chatId].CallbackQuery.Data = $"become_tutor_wait_description_university&{fullname}&{subjId}&{categoryId}";
