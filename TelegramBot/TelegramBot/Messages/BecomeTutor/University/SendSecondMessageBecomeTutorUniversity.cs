...
namespace TelegramBot.Messages.BecomeTutor.University
{
    ...
    {
        public static async Task SendSecondMessageBecomeTutorUniversity(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
        {
            ...
            var inlineKeyBoardButtons = categories.Select(category =>
                new[]{ InlineKeyboardButton.WithCallbackData(
                    text: category.Value,
                    callbackData: $"become_tutor_uni_subj&{category.Key}")
                }).ToList();
